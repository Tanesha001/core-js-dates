...
function getDayName(date) {
  ...
  return res.toLocaleDateString('en-US', { weekday: 'long' });
}
...
function getNextFriday(date) {
  const countWeek = date.getDay();
  const day = date.getDate();
  let toFriday = 5 - countWeek;
  if (toFriday <= 0) {
    toFriday += 7;
  }
  return new Date(date.setDate(toFriday + day));
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  while (newDate.getMonth() === month - 1) {
    if (newDate.getDay() === 0 || newDate.getDay() === 6) count += 1;
...
function getWeekNumberByDate(date) {
  ...
  const msecondsByDay = 86400000;
  const shift = firstDayOfYear.getDay() === 7 ? 1 : 0;

  start.setDate(firstDayOfYear.getDate() + shift);
  end.setDate(date.getDate() + ((-date.getDay() + 7) % 7));

  const difference = Math.floor((end - start) / msecondsByDay);
